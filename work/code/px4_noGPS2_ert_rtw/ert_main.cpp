...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(px4_noGPS2_M) == (NULL));
  while (runModel) {
    ...
    // Get model outputs here
    stopRequested = !((rtmGetErrorStatus(px4_noGPS2_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(px4_noGPS2_M, "stopping the model");
  runModel = 0;
}
...
int px4_simulink_app_task_main (int argc, char *argv[])
{
  ...
  rtmSetErrorStatus(px4_noGPS2_M, 0);

  // Initialize model
  px4_noGPS2_initialize();
